defmodule LvtestWeb.HTML.Tag do
  @moduledoc ~S"""
  ...
  def content_tag(name, content, attrs) when is_atom(name) do
    Phoenix.View.render(__MODULE__, "content_tag.html", name: name, content: content, attrs: attrs)
  end

  def render("content_tag.html", %{name: _, content: _, attrs: _} = assigns) do
    ~L"""
    <<%= @name %><%= build_attrs(@name, @attrs) %>><%= @content %></<%= @name %>>
    """
  end
  ...
  defp tag_attrs(attrs) do
    assigns = %{attrs: attrs}
    ~L"""
    <%= for a <- @attrs do %> <%= one_tag_attr(a) %><% end %>
    """
  end

  defp one_tag_attr({k, v}) do
    assigns = %{k: k, v: v}
    ~L"""
    <%= @k %>="<%= @v %>"
    """
